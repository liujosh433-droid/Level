...
| Router inline extraction (priority, person, reminder) | `core/schemas/chat.py`, `core/agents/chat_router.py`, `api/routes/chat.py` | Router's structured output now includes `inline_priority`, `inline_person_edit`, `inline_reminder`. Filled during the single Flash routing call; dispatcher writes straight to Firestore and skips the specialist LLM call (`PriorityAgent`, `PersonEditAgent`, `ReminderAgent`) entirely. **Cuts these intents from 2 LLM calls to 1** — was the main source of the ~30s tail under quota pressure ("elder care with mom takes precedent"). Specialists still run as fallback when the router isn't confident, so ambiguous / multi-value / novel phrasings don't lose accuracy. Router context is enriched with `<people>` + `<negatives>` so the inline extractor sees the same signals the specialists did. |
